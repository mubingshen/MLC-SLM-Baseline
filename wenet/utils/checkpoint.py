...
def load_projector(model: torch.nn.Module, path: str) -> dict:
    rank = int(os.environ.get('RANK', 0))
    logging.info('[Rank {}] Projector Checkpoint: loading from projector checkpoint {}'.format(
        rank, path))
    ...
    for k in checkpoint.keys():
        if 'encoder_projector' in k:
            nk = '.'.join(k.split('.')[1:])
            checkpoint_encoder_projector[nk] = checkpoint[k]
    missing_keys, unexpected_keys = model.encoder_projector.load_state_dict(checkpoint_encoder_projector, strict=False)
    if rank == 0:
    ...
    checkpoint_trainable_prompts = {}
    for k in checkpoint.keys():
        if 'trainable_prompts' in k:
            nk = '.'.join(k.split('.')[1:])
            checkpoint_trainable_prompts[nk] = checkpoint[k]
    missing_keys, unexpected_keys = model.trainable_prompts.load_state_dict(checkpoint_trainable_prompts, strict=False)
    if rank == 0:
        for key in missing_keys:
            logging.info("missing tensor: {}".format(key))
        for key in unexpected_keys:
