...
if __name__ == '__main__':
    ...
    with open(args.output, "w") as f:
        for line in tqdm(open(args.input, "r").readlines()):
            ...
            spkid = line.strip().split()[2]
            begin_time = line.strip().split()[3]
            end_time = line.strip().split()[4]
            if 'Japanese' in utt or 'Korean' in utt or 'Thai' in utt:
                text = ' '.join(line.strip().split()[5:])
                text_tn = add_space_between_chars(text)
            else:
                text_tn = ' '.join(line.strip().split()[5:])
            f.write(f"{utt} {channel} {spkid} {begin_time} {end_time} {text_tn}\n")
